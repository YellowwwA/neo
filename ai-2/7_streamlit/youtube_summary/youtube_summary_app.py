...
from youtube_transcript_api.errors import NoTranscriptFround, TranscriptsDisabled
...
def summarize_youtube_video(video_url, selected_lang, trans_method):
    ...
    try:
        ...
        st.error('이 도영상의 자막이 비활성화되어 있습니다..')
        return
    except Exception as e:
        st.error(f'자막을 가져오는 중 오류가 발생했습니다. : {str(e)}')
        return
    
    token_num = calc_token_num(yt_trnacript)
    ...
    if selected_lang == '영어':
        if trans_method == 'OpenAI':
            ...
            trans_result = my_text_sum.translage_english_to_korean_using_deepl(final_summary)
